      ...
      <UserContainer>
        <UserContentContainer>
          ...
          <section>
            <UserProfile>
              <UserProfileContainer>
                <UserProfilePhoto image={user.image || '/images/default-user.png'} />
                <div>
                  <h1>{user.firstName} {user.lastName}</h1>
                  <h2>{user.role === "ADMIN" ? "Administrador" : "Usuário padrão"}</h2>
                </div>
              </UserProfileContainer>
            </UserProfile>
            <UserContent>
                ...
                <ActionButtonsContainer>
                  {
                    ...
                    session.user.role === "ADMIN" ? (
                      ...
                      <button onClick={() => setShowChangePasswordModal(true)} className="edit">
                        Alterar senha
                        <FiLock />
                      </button>
                    )
...
export const getServerSideProps: GetServerSideProps = async (ctx) => {
  ...
  const passwordMask = base64.decode(response.data.password).split('').map(() => "*")
